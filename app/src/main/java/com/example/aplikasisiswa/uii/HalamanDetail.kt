...
import com.example.aplikasisiswa.model.DetailsViewModel
import com.example.aplikasisiswa.model.ItemDetailsUiState
import com.example.aplikasisiswa.model.PenyediaViewModel
import com.example.aplikasisiswa.model.toSiswa
import com.example.aplikasisiswa.navigasi.DestinasiNavigasi
import kotlinx.coroutines.launch

object DetailsDestination: DestinasiNavigasi{
    override val route = "item_details"
    override val titileRes = R.string.detail_siswa
...
fun DetailScreen(
    ...
    Scaffold (
        topBar = {
            SiswaTopAppBar(
                title = stringResource(DetailsDestination.titileRes),
...
private fun ItemDetailsBody(
    ...
    Column(
        ...
        var deleteConfirmationRequiered by rememberSaveable {mutableStateOf(false) }
        ItemDetails(
        ...
        OutlinedButton(
            onClick = {deleteConfirmationRequiered = true},
            shape = MaterialTheme.shapes.small,
            modifier = Modifier.fillMaxWidth()
        ) {
            Text(stringResource(id = R.string.delete))
        }
        if (deleteConfirmationRequiered){
            DeleteConfirmationDialog(
                onDeleteConfirm = {
                    deleteConfirmationRequiered = false
                    onDelete()
                },
                onDeleteCancel = { deleteConfirmationRequiered = false },
                modifier = Modifier.padding(dimensionResource(id = R.dimen.padding_medium))
            )
        }
    }
